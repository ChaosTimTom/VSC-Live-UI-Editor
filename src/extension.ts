...
import { detectPackageManager, getFreePort, pickViteAppRoot, waitForHttpReady } from './appMode/viteUtils';
...
`;
	...
	const disposableAppMode = vscode.commands.registerCommand('liveUI.openAppMode', async () => {
		const appRoot = await pickViteAppRoot();
		if (!appRoot) return;
		...
		let launchMode: 'external' | 'integrated' | 'existing' = 'integrated';
		if (process.platform === 'win32') {
			const pick = await vscode.window.showQuickPick(
				[
					{ label: 'External window', description: 'Start dev server in a separate cmd window', value: 'external' as const },
					{ label: 'Integrated terminal', description: 'Start dev server in VS Code terminal', value: 'integrated' as const },
					{ label: 'Use existing URL', description: 'I already have the dev server running', value: 'existing' as const },
				],
		...
		let ready = await waitForHttpReady(origin, launchMode === 'integrated' ? 60000 : 45000);
		if (!ready) {
			const pick = await vscode.window.showErrorMessage(
				`Live UI Editor: Timed out waiting for dev server at ${origin}.`,
				{ modal: false },
				'Retry',
			...
			if (pick === 'Use Existing URL') {
				...
				if (!url) return;
				ready = await waitForHttpReady(url, 15000);
				if (!ready) {
		...
		currentAppProxy = await startInjectedProxyServer({
			targetOrigin: origin,
			injectedScript: injectedClientScript,
			logger: (line) => { output.appendLine(line); },
		});
