...
`;
	...
	const disposableAppMode = vscode.commands.registerCommand('liveUI.openAppMode', async () => {
		...
		if (inlineRiskWarnKey) {
			const saved = context.workspaceState.get<string>(inlineRiskWarnKey);
			if (saved === 'ask' || saved === 'auto' || saved === 'inline' || saved === 'off') {
				inlineRiskWarnMode = saved;
			}
		}
		if (stylePrefKey) {
			const saved = context.workspaceState.get<string>(stylePrefKey);
		...
		type AppModeApplyItemReport = {
			...
			elementId?: string;
			ok: boolean;
			method?: 'elementId' | 'location' | 'class' | 'tailwind';
			reason?: string;
			error?: string;
		};
		...
		async function setCssTargetUri(next: vscode.Uri | undefined): Promise<void> {
			cssTargetUri = next;
			if (!panel) return;
			const label = cssTargetUri ? vscode.workspace.asRelativePath(cssTargetUri, false) : '';
			try { panel.webview.postMessage({ command: 'appModeCssTarget', label }); } catch {}
		}
		function publishStyleAdapterInfo(effective?: StyleAdapterId, reason?: string): void {
			if (!panel) return;
			try {
		...
		function resolveEffectiveStyleAdapter(): { effective: StyleAdapterId; reason: string } {
		...
		async function ensureCssTargetUri(): Promise<vscode.Uri | undefined> {
			if (cssTargetUri) return cssTargetUri;
		...
		function adapterTryOrder(pref: StyleAdapterPreference): StyleAdapterId[] {
			if (pref === 'inline') return ['inline'];
			if (pref === 'tailwind') return ['tailwind', 'inline'];
			if (pref === 'cssClass') return ['cssClass', 'inline'];
			// auto
			if (styleDetection?.tailwind.present) return ['tailwind', 'cssClass', 'inline'];
				...
				for (const e of fileEdits) {
					...
					try {
						if (e.kind === 'style') {
							...
							let changed = false;
							let method: 'elementId' | 'location' | 'class' | 'tailwind' | undefined;
							...
							for (const adapter of tryOrder) {
								if (adapter === 'tailwind') {
									...
									const tokens = tailwindTokensFromStylePatch(style);
									if (tokens.length === 0) {
		...
		async function createAppModePanel(): Promise<vscode.WebviewPanel> {
			...
			created.webview.onDidReceiveMessage(async (message: unknown) => {
				...
				if (message.command === 'setStyleAdapter') {
					...
					const resolved = resolveEffectiveStyleAdapter();
					publishStyleAdapterInfo(resolved.effective, resolved.reason);
					if (resolved.effective === 'cssClass') void ensureCssTargetUri();
					return;
				}
				...
				if (message.command === 'fixTargeting') {
					...
					try {
						...
						if (res.ok) {
							try { created.webview.postMessage({ command: 'appModeHint', text: 'Stable IDs enabled. Reloading… after reload, click the element again to select it.' }); } catch {}
				...
				if (message.command === 'applyPendingEdits') {
					...
					const hasRiskyInlineStyles = (() => {
						if (styleAdapterPref !== 'inline') return false;
						const risky = new Set([
					...
					if (hasRiskyInlineStyles && inlineRiskWarnMode !== 'off') {
						if (inlineRiskWarnMode === 'auto') {
							...
							const choice = await vscode.window.showWarningMessage(
								'Live UI Editor (App Mode): You are about to apply inline layout styles (e.g. width/height/position/transform). Inline styles override responsive CSS and can break mobile layouts.\n\nTip: Use “Auto” to write Tailwind tokens or a CSS class rule instead.',
								{ modal: true },
								'Switch to Auto (recommended)',
								'Continue Inline',
								'Always switch to Auto',
								'Always continue Inline',
								"Don't warn again",
								'Cancel'
							);
							...
							if (choice === "Don't warn again") {
								...
								if (inlineRiskWarnKey) void context.workspaceState.update(inlineRiskWarnKey, inlineRiskWarnMode);
							}
					...
					if (hasUnsafe && !message.forceUnsafe) {
						const choice = await vscode.window.showWarningMessage(
							'Live UI Editor (App Mode): Apply is blocked because the selection is not using Stable IDs (targeting may be ambiguous).',
							{ modal: true },
							'Fix targeting',
							'Apply anyway (unsafe)',
							'Cancel'
						...
						if (choice === 'Fix targeting') {
							try { created.webview.postMessage({ command: 'appModeHint', text: 'Enable Stable IDs, then reselect the element.' }); } catch {}
						...
						if (choice !== 'Apply anyway (unsafe)') return;
					}
